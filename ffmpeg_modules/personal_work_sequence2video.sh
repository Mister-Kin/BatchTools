...
personal_work_sequence2video() {
    ...
    ffmpeg_no_banner -r "$sequence_video_fps" -f image2 -i "$input_file" -r "$sequence_video_fps" -c:v libx264 -crf:v "$video_crf" -preset:v "$video_preset" -maxrate:v "$video_max_bitrate" -bufsize:v "$video_bufsize" -vf "$filter_effect" -bsf:v filter_units=remove_types=6 "output.mp4"
    ((operation_count++))
    show_progress_bar "1" "$operation_count"
