...
audio_metadata_set_artist() {
    local output_path="audio_metadata_set_artist"
    local feature_name feature_intro feature_note
    feature_name="设置音频文件的歌手名"
    feature_intro="设置路径下的mp3文件或者m4a文件或者flac文件元数据标签的歌手名$(description_append_intro "设置元数据标签-歌手名")"
    ...
    local audio_artist
    audio_artist=$(input_anything "请输入元数据标签-歌手名")
    if [ $? -eq 10 ]; then
        return 20
    fi
    draw_line_echo "~"
